...
if __name__ == "__main__":
	...
	with tf.variable_scope('cell'):
		headline_output, headline_states = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, 
			tf.nn.dropout(headline_embedded_tensor, dropout_placeholder_input), dtype = tf.float32, 
	...
	with tf.variable_scope('cell', reuse=True):
		article_output, article_states = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, 
			article_embedded_tensor, dtype = tf.float32, 
			sequence_length = article_lengths_placeholder, initial_state_fw = headline_states[0],
