...
class ModelHelper(object):
    ...
    """
    def __init__(self, tok2id, max_length_articles, max_length_headlines):
        self.tok2id = tok2id
        self.START = [tok2id[START_TOKEN], tok2id[P_CASE + "aa"]]
        self.END = [tok2id[END_TOKEN], tok2id[P_CASE + "aa"]]
        # self.max_length = max_length
        self.max_length_articles = max_length_articles
        self.max_length_headlines = max_length_headlines


    def vectorize_example(self, headline, article, labels=None):
        headline_ = [self.tok2id.get(normalize(word), self.tok2id[UNK]) for word in headline]
        article_ = [self.tok2id.get(normalize(word), self.tok2id[UNK]) for word in article]
#        sentence_ = [[self.tok2id.get(normalize(word), self.tok2id[UNK]), self.tok2id[P_CASE + casing(word)]] for word in sentence]
        if labels:
            labels_ = LBLS.index(labels)#[LBLS.index(l) for l in labels]
            return headline_, article_, labels_
        else:
            return headline_, article_, [LBLS[-1] for _ in sentence]

    def vectorize(self, data):
        return [self.vectorize_example(headline, article, labels) for headline, article, labels in data]
...
#        tok2id = build_dict((normalize(word) for sentence, _ in data for word in sentence), offset=1, max_words=10000)
        ...
        # not sure if this code is right. also, should we increase max words count? 
        tok2id = build_dict((normalize(word) for article, headline, _ in data for word in article + headline), offset=1, max_words=80000)
        ...
        max_length_headlines = max(len(sentence) for sentence, _, _ in data)
        max_length_articles = max(len(sentence) for _, sentence, _ in data)

        return cls(tok2id, max_length_headlines, max_length_articles)
...
def load_and_preprocess_data(args):
    ...
    allData = [(s['Headline'].split(), s['Stance']) for s in stances]
    train = allData[:len(allData)*4/5]
    dev = allData[(len(allData)*4/5 + 1):]


    logger.info("Done. Read %d sentences", len(allData))
...
def load_embeddings(helper, vocabPath = "data/vocab.txt", vectorPath = "data/wordVectors.txt", wordFirst = False, embed_size = 50):
    embeddings = np.array(np.random.randn(len(helper.tok2id) + 1, embed_size), dtype=np.float32)
    embeddings[0] = 0.

    # temp
    vocab = open(vocabPath)
    vectors = open(vectorPath)

    for word, vec in load_word_vector_mapping(vocab, vectors, wordFirst).items():
        word = normalize(word)
