...
if __name__ == "__main__":
	...
	if sys.argv[1] == "RNN":
		headline_cell_fw1 = tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size)
		headline_cell_bw1 = tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size)
		headline_cell_fw2 = tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size)
		headline_cell_bw2 = tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size)
	elif sys.argv[1] == "GRU":
		headline_cell_fw1 = tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size)
		headline_cell_bw1 = tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size)
		headline_cell_fw2 = tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size)
		headline_cell_bw2 = tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size)
	elif sys.argv[1] == "LSTM":
		headline_cell_fw1 = tf.nn.rnn_cell.LSTMCell(Config.headline_hidden_size)
		headline_cell_bw1 = tf.nn.rnn_cell.LSTMCell(Config.headline_hidden_size)
		headline_cell_fw2 = tf.nn.rnn_cell.LSTMCell(Config.headline_hidden_size)
		headline_cell_bw2 = tf.nn.rnn_cell.LSTMCell(Config.headline_hidden_size)
	else:
	...
	with tf.variable_scope('headline_cell'):
		headline_output, headline_states = tf.nn.bidirectional_dynamic_rnn(tf.nn.rnn_cell.MultiRNNCell([headline_cell_fw1, headline_cell_fw2]),
			tf.nn.rnn_cell.MultiRNNCell([headline_cell_bw1, headline_cell_bw2]), 
			headline_embedded_tensor, dtype = tf.float32, sequence_length = headline_lengths_placeholder) 

	# get the cell for articles 
	if sys.argv[1] == "RNN":
		article_cell_fw1 = tf.nn.rnn_cell.BasicRNNCell(Config.article_hidden_size)
		article_cell_bw1 = tf.nn.rnn_cell.BasicRNNCell(Config.article_hidden_size)
		article_cell_fw2 = tf.nn.rnn_cell.BasicRNNCell(Config.article_hidden_size)
		article_cell_bw2 = tf.nn.rnn_cell.BasicRNNCell(Config.article_hidden_size)
	elif sys.argv[1] == "GRU":
		article_cell_fw1 = tf.nn.rnn_cell.GRUCell(Config.article_hidden_size)
		article_cell_bw1 = tf.nn.rnn_cell.GRUCell(Config.article_hidden_size)
		article_cell_fw2 = tf.nn.rnn_cell.GRUCell(Config.article_hidden_size)
		article_cell_bw2 = tf.nn.rnn_cell.GRUCell(Config.article_hidden_size)
	elif sys.argv[1] == "LSTM":
		article_cell_fw1 = tf.nn.rnn_cell.LSTMCell(Config.article_hidden_size)
		article_cell_bw1 = tf.nn.rnn_cell.LSTMCell(Config.article_hidden_size)
		article_cell_fw2 = tf.nn.rnn_cell.LSTMCell(Config.article_hidden_size)
		article_cell_bw2 = tf.nn.rnn_cell.LSTMCell(Config.article_hidden_size)

	# create the article cell
	with tf.variable_scope('article_cell'):
		article_output, article_states = tf.nn.bidirectional_dynamic_rnn(tf.nn.rnn_cell.MultiRNNCell([article_cell_fw1, article_cell_fw2]),
			tf.nn.rnn_cell.MultiRNNCell([article_cell_bw1, article_cell_bw2]), 
	...
	if sys.argv[1] != "LSTM":
		...
		headline_state = headline_states[0][1][1] + headline_states[1][1][1]
		article_state = article_states[0][1][1] + article_states[1][1][1]
