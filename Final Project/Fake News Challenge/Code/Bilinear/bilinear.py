...
if __name__ == "__main__":
	...
	headlineConcatenatedHidden = tf.concat(2, [headline_output[0], headline_output[1]]) 
	normalizedHCH = tf.nn.l2_normalize(headlineConcatenatedHidden, dim = 2) 
	...
	Bilinear_Matrix = tf.get_variable('Bilinear_Matrix', (2*Config.article_hidden_size, 2*Config.headline_hidden_size), 
		initializer = tf.contrib.layers.xavier_initializer())
	weights1 = tf.matmul(Bilinear_Matrix, tf.transpose(tf.reshape(normalizedHCH, shape = [-1, 2*Config.headline_hidden_size]), perm = [1, 0]))
	weights2 = tf.reshape(weights1, [-1, 2*Config.headline_hidden_size, Config.max_length_titles])
	weights = tf.batch_matmul(articleConcatenatedHidden, weights2)
	...
	with tf.variable_scope('conditional_cell'):
		conditional_output, conditional_states = tf.nn.bidirectional_dynamic_rnn(conditional_cell_fw, conditional_cell_bw, 
			conditionalInput, dtype = tf.float32, sequence_length = batch_size_placeholder, time_major = False, swap_memory = True)
