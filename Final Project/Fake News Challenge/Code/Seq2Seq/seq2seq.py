...
import core_rnn_cell_impl

#from rnn_cell_impl import _RNNCell
...
#	train = allData[:int(len(allData)*4/5)]
#	dev = allData[int(len(allData)*4/5):]
...
if __name__ == "__main__":
	...
	#placeholders for sequence 2 sequence
	#encode_input = 
	...
	#cell = tf.nn.rnn_cell.BasicRNNCell(Config.article_hidden_size)
	#cell = Config.core_rnn_cell.RNNCell(Config.article_hidden_size)
	#cell = tf.contrib.rnn.MultiRNNCell(Config.article_hidden_size)
	#cell = core_rnn_cell.RNNCell(Config.article_hidden_size)
	cell = core_rnn_cell.RNNCell()

	outputs, state = seq2seqcode.embedding_attention_seq2seq(article_embedded_tensor, headline_embedded_tensor, cell, Config.max_length_articles, Config.max_length_titles, Config.embed_size)
...
#			initial_state = article_state)
	...
	#combined_hidden = tf.nn.dropout(state, dropout_placeholder)
	# compute
	last_hidden = tf.nn.relu(tf.matmul(state, W) + b)
	...
	# compute
	output = tf.matmul(combined_hidden, W) + b
