...
if __name__ == "__main__":
	...
	#how to concenate this?
	headline_state = tf.concat(1, [headline_states[0], headline_states[1]])
	#article_state = tf.transpose((tf.pack(article_states[0]), [1, 2, 0])), tf.transpose((tf.pack(article_states[1]), [1, 2, 0])) 
	#else:
	#	article_state = article_states[0][1] + article_states[1][1]
	#combined_hidden = article_state

	# define variables for multiplying
	W = tf.get_variable('W', (2 * Config.headline_hidden_size, Config.final_state_size), 
	...
	# compute
	last_hidden = tf.nn.relu(tf.matmul(headline_state, W) + b)
