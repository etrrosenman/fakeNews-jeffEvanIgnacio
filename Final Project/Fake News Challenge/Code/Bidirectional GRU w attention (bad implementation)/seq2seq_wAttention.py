...
if __name__ == "__main__":
	...
	if sys.argv[1] == "RNN":
		...
		article_cell_fw = GRUAttnCell(Config.article_hidden_size, headline_output[0])
		article_cell_bw = GRUAttnCell(Config.article_hidden_size, headline_output[1])
	elif sys.argv[1] == "LSTM":
	...
	with tf.variable_scope('article_cell'):
		article_output, article_states = tf.nn.bidirectional_dynamic_rnn(article_cell_fw, article_cell_bw, 
			...
			initial_state_fw = headline_states[0], initial_state_bw = headline_states[1])  
	...
	for epoch in range(Config.epochs): 
		...
		for batch in batches:
			...
			# run session
			_, local_loss, astate = session.run([train_op, loss, article_state], feed_dict = feed_dict)
