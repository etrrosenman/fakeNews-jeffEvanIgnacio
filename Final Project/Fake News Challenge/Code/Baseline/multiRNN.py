...
class Config:
	...
	batch_size = 32
	embedding_size = 50 # try a larger embedding
...
if __name__ == "__main__":
	...
	pretrained_embeddings = load_embeddings(helper, "data/vocab.txt")
	...
	if sys.argv[1] == "RNN":
		headline_cell1 = tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size)
		headline_cell2 = tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size)
		headline_cell3 = tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size)
	elif sys.argv[1] == "GRU":
		headline_cell1 = tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size)
		headline_cell2 = tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size)
		headline_cell3 = tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size)
	else:
		print "must specify cell type!"
		exit()
	
	head = tf.nn.rnn_cell.MultiRNNCell([headline_cell1, headline_cell2, headline_cell3])
	...
	with tf.variable_scope('headline_cell'):
		...
		headline_output, headline_state = tf.nn.dynamic_rnn(head, headline_embedded_tensor, dtype = tf.float32,
			sequence_length = headline_lengths_placeholder) 

	# get the cell for articles 
	if sys.argv[1] == "RNN":
		article_cell1 = tf.nn.rnn_cell.BasicRNNCell(Config.article_hidden_size)
		article_cell2 = tf.nn.rnn_cell.BasicRNNCell(Config.article_hidden_size)
		article_cell3 = tf.nn.rnn_cell.BasicRNNCell(Config.article_hidden_size)
	elif sys.argv[1] == "GRU":
		article_cell1 = tf.nn.rnn_cell.GRUCell(Config.article_hidden_size)
		article_cell2 = tf.nn.rnn_cell.GRUCell(Config.article_hidden_size)
		article_cell3 = tf.nn.rnn_cell.GRUCell(Config.article_hidden_size)

	art = tf.nn.rnn_cell.MultiRNNCell([article_cell1, article_cell2, article_cell3])

	# article RNN
	with tf.variable_scope('article_cell'):
		article_output, article_state = tf.nn.dynamic_rnn(art, article_embedded_tensor, dtype = tf.float32, 
			sequence_length = article_lengths_placeholder)

	# concatenate the hidden layers (still confused...)
	combined_hidden = tf.concat(1, [headline_state[2], article_state[2]]) 
