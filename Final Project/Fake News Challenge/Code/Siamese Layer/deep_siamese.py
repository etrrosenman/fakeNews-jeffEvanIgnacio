...
if __name__ == "__main__":
	...
	# get the cell 
	if sys.argv[1] == "RNN":
		cell_fw1 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_bw1 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_fw2 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_bw2 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.BasicRNNCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
	elif sys.argv[1] == "GRU":
		cell_fw1 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_bw1 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_fw2 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_bw2 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.GRUCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
	elif sys.argv[1] == "LSTM":
		cell_fw1 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.LSTMCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_bw1 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.LSTMCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_fw2 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.LSTMCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
		cell_bw2 = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.LSTMCell(Config.headline_hidden_size), dropout_placeholder, dropout_placeholder)
	else:
	...
	with tf.variable_scope('cell'):
		headline_output, headline_states = tf.nn.bidirectional_dynamic_rnn(tf.nn.rnn_cell.MultiRNNCell([cell_fw1, cell_fw2]),
			tf.nn.rnn_cell.MultiRNNCell([cell_bw1, cell_bw2]), tf.nn.dropout(headline_embedded_tensor, dropout_placeholder), 
			dtype = tf.float32, sequence_length = headline_lengths_placeholder)  

	# process the article
	with tf.variable_scope('cell', reuse=True):
		article_output, article_states = tf.nn.bidirectional_dynamic_rnn(tf.nn.rnn_cell.MultiRNNCell([cell_fw1, cell_fw2]),
			tf.nn.rnn_cell.MultiRNNCell([cell_bw1, cell_bw2]), tf.nn.dropout(article_embedded_tensor, dropout_placeholder), 
			dtype = tf.float32, sequence_length = article_lengths_placeholder)  
	...
	if sys.argv[1] != "LSTM":
		...
		headline_state = tf.concat(1, [headline_states[0][1][1], headline_states[1][1][1]])
		article_state = tf.concat(1, [article_states[0][1][1], article_states[1][1][1]])
...
#			print epoch_loss/float(batchCounter), local_loss, batchCounter 
		...
		# save the model
		if epoch == Config.epochs - 1:
			saver.save(session, 'savedModel_Sharing/')
