...
if __name__ == "__main__":
	...
	pretrained_embeddings = load_embeddings(helper, "data/6bVocab.txt", "data/glove.6B.50d.txt", True)
...
#	combined_avg = tf.concat(1, [headline_mean, article_mean]) 
	diffs = tf.subtract(headline_mean, article_mean)

	# define variables for multiplying
	# W1 = tf.get_variable('W1', (2*Config.embedding_size, Config.hidden_size), 
	...
	W = tf.get_variable('W', (Config.embed_size, Config.num_classes), 
		initializer = tf.contrib.layers.xavier_initializer())
	b = tf.get_variable('b', Config.num_classes, initializer = tf.zeros_initializer)
	output = tf.matmul(diffs, W) + b
...
#			labels_batch = np.array([x[2] for x in batch])
			...
			confusion += confusion_matrix(predictions, labels_batch, labels = range(2))
