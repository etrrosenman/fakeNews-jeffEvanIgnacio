...
class NERModel(Model):
    ...
    def evaluate(self, sess, examples, examples_raw):
        ...
        results = self.output(sess, examples_raw, examples)

        predictedLabels = [v[2] for v in results]
        trueLabels = [v[1] for v in results] 
        p = sum([predictedLabels[i] == trueLabels[i] for i in range(len(predictedLabels))])/float(len(trueLabels))
        r = 0.0 #temp
        f1 = 0.0 #temp 
 ...
 #           correct_preds += int(labels == labels_)
  #          total_preds += 1
            # t()
        ...
        p = correct_preds / float(total_preds) if correct_preds > 0 else 0
        r = correct_preds / float(total_correct) if correct_preds > 0 else 0
...
#        logger.debug("Token-level scores:\n" + token_cm.summary())
#        logger.info("Entity level P/R/F1: %.2f/%.2f/%.2f", *entity_scores)
        logger.info("Entity level P: %.2f", entity_scores[0])
