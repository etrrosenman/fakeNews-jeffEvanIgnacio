...
class Config:
	...
	conditional_hidden_size = 100 #originally 100

	batch_size = 32
	embed_size = 200 # try a larger embedding
	max_length_titles = 40
	max_length_articles = 250 #changed from 700
	num_classes = 4
	epochs = 10
	lr = 0.001 #original is .001
	dropout = 1.0
	final_state_size = 50 
...
if __name__ == "__main__":
	...
	# get the cell for the conditional representation 
	if sys.argv[1] == "RNN":
		conditional_cell_fw = tf.nn.rnn_cell.BasicRNNCell(Config.conditional_hidden_size)
		conditional_cell_bw = tf.nn.rnn_cell.BasicRNNCell(Config.conditional_hidden_size)
	elif sys.argv[1] == "GRU":
		conditional_cell_fw = tf.nn.rnn_cell.GRUCell(Config.conditional_hidden_size)
		conditional_cell_bw = tf.nn.rnn_cell.GRUCell(Config.conditional_hidden_size)
	elif sys.argv[1] == "LSTM":
		conditional_cell_fw = tf.nn.rnn_cell.LSTMCell(Config.conditional_hidden_size)
		conditional_cell_bw = tf.nn.rnn_cell.LSTMCell(Config.conditional_hidden_size)

	# processs the conditional cell 
	headlineConcatenatedHidden = tf.concat(2, [headline_output[0], headline_output[1]]) # is this okay with LSTM 
	normalizedHCH = tf.nn.l2_normalize(headlineConcatenatedHidden, dim = 2) # don't use L2 normalized version

	articleConcatenatedHidden = tf.concat(2, [article_output[0], article_output[1]])
	articleConcatenatedHidden = tf.nn.l2_normalize(articleConcatenatedHidden, dim = 2)
	weights = tf.batch_matmul(articleConcatenatedHidden, tf.transpose(normalizedHCH, perm = [0, 2, 1]))
	weights = tf.nn.softmax(weights, 1)
	articleContext = tf.transpose(tf.matmul(tf.transpose(articleConcatenatedHidden, perm = [0, 2, 1]), weights), [0, 2, 1])
	
	#possible dropout here
	#headlineConcatenatedHidden = tf.nn.dropout(headlineConcatenatedHidden, dropout_placeholder)
	#articleContext = tf.nn.dropout(articleContext, dropout_placeholder)

	conditionalInput = tf.concat(2, [headlineConcatenatedHidden, articleContext])

	with tf.variable_scope('conditional_cell'):
		conditional_output, conditional_states = tf.nn.bidirectional_dynamic_rnn(conditional_cell_fw, conditional_cell_bw, 
			conditionalInput, dtype = tf.float32, sequence_length = headline_lengths_placeholder, time_major = False, swap_memory = True)

	# define variables for multiplying
	conditional_output_concat = tf.concat(1, [conditional_states[0][1], conditional_states[1][1]])
	W = tf.get_variable('W', (2*Config.conditional_hidden_size, Config.final_state_size), 
		initializer = tf.contrib.layers.xavier_initializer())
	b = tf.get_variable('b', Config.final_state_size, initializer = tf.zeros_initializer)

	#replaced last dropout here:
	#conditional_output_concat = tf.nn.dropout(conditional_output_concat, dropout_placeholder)

	# compute
	last_hidden = tf.nn.relu(tf.matmul(conditional_output_concat, W) + b)

	#last possible option for dropout here
	last_hidden = tf.nn.dropout(last_hidden, dropout_placeholder)
	...
	saver = tf.train.Saver()
	session = tf.Session() 
	saver.restore(session, '../Conditioning_Attention/Saved_Model 5.4.17/')
	...
	with open('./results_namedEntityMatching_secondModel.csv', 'wb') as csvfile:
