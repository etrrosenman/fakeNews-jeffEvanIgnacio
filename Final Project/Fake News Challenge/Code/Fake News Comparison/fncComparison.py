...
def load_and_preprocess_data(debug = False):
	...
	# use as our "real" baseline the pre-election NYT articles 
	preElectionNYTArticles = [nytArticles[i].split() for i in nytIndices if datetime.strptime(nytDates[i], '%m/%d/%y') < 
		datetime.strptime('11/8/16', '%m/%d/%y') and datetime.strptime(nytDates[i], '%m/%d/%y') >= datetime.strptime('11/1/16', '%m/%d/%y')]

	preElectionNYTHeadlines = [nytHeadlines[i].split() for i in nytIndices if datetime.strptime(nytDates[i], '%m/%d/%y') < 
		datetime.strptime('11/8/16', '%m/%d/%y') and datetime.strptime(nytDates[i], '%m/%d/%y') >= datetime.strptime('11/1/16', '%m/%d/%y')]


	# use WaPo for real headlines 
	preElectionRealHeadlines = [wapoHeadlines[i].split() for i in wapoIndices if datetime.strptime(wapoDates[i], '%m/%d/%y') < datetime.strptime('11/8/16', '%m/%d/%y') and 
		datetime.strptime(wapoDates[i], '%m/%d/%y') >= datetime.strptime('11/1/16', '%m/%d/%y')]

	preElectionRealArticles = [wapoArticles[i].split() for i in wapoIndices if datetime.strptime(wapoDates[i], '%m/%d/%y') < datetime.strptime('11/8/16', '%m/%d/%y') and 
		datetime.strptime(wapoDates[i], '%m/%d/%y') >= datetime.strptime('11/1/16', '%m/%d/%y')]

	preElectionFakeHeadlines = [fakeHeadlines[i].split() for i in fakeIndices if datetime.strptime(fakeDates[i], '%m/%d/%y') < datetime.strptime('11/8/16', '%m/%d/%y') and 
		datetime.strptime(fakeDates[i], '%m/%d/%y') >= datetime.strptime('11/1/16', '%m/%d/%y')]

	preElectionFakeArticles = [fakeArticles[i].split() for i in fakeIndices if datetime.strptime(fakeDates[i], '%m/%d/%y') < datetime.strptime('11/8/16', '%m/%d/%y') and 
		datetime.strptime(fakeDates[i], '%m/%d/%y') >= datetime.strptime('11/1/16', '%m/%d/%y')]


	# # pull a subset of fake headlines and pair them with all real articles
	# np.random.seed(2017)
	# np.random.shuffle(preElectionFakeHeadlines)
	# fakeSample =  preElectionFakeHeadlines[:1000]
	# sample1 = [(f[:min(len(f), 40)], u[:min(len(u), 40)], 'unrelated') for f in fakeSample for u in preElectionNYTArticles]
	
	# # pull a subset of real headlines and pair them with all real articles
	# np.random.seed(2017)
	# np.random.shuffle(preElectionRealHeadlines)
	# realSample =  preElectionRealHeadlines[:1000]
	# sample2 = [(f[:40], u[:40], 'unrelated') for f in realSample for u in preElectionNYTArticles]

	# pull a subset of fake articles and pair them with all real articles
	np.random.seed(2017)
	np.random.shuffle(preElectionFakeArticles)
	fakeSample =  preElectionFakeArticles[:1000]
	sample1 = [(r[:min(len(r), 40)], f[:min(len(f), 40)], 'unrelated') for \
		f in fakeSample for r in preElectionNYTHeadlines]
	
	# pull a subset of real articles and pair them with all real headlines
	np.random.seed(2017)
	np.random.shuffle(preElectionRealArticles)
	realSample =  preElectionRealArticles[:1000]
	sample2 = [(r[:min(len(r), 40)], u[:min(len(u), 40)], 'unrelated') for \
		u in preElectionRealArticles for r in preElectionNYTHeadlines]
	...
	Config.batch_size = len(preElectionNYTArticles)
