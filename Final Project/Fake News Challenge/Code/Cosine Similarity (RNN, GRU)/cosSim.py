...
class Config:
	...
	headline_hidden_size = 5
	article_hidden_size = 5
...
#			initial_state = article_state)
	...
	# compute the cosine similarity of the states
	cosSim = tf.reduce_sum(tf.multiply(headline_state, article_state), axis = 1)/\
			 tf.sqrt(tf.reduce_sum(tf.multiply(headline_state, headline_state), axis = 1))/\
			 tf.sqrt(tf.reduce_sum(tf.multiply(article_state, article_state), axis = 1))

	# define variables for multiplying
	W = tf.get_variable('W', (Config.headline_hidden_size + Config.article_hidden_size, Config.num_classes), 
	...
	total_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(tf.transpose((tf.square(cosSim), -tf.square(cosSim))), labels_placeholder)
#	total_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(output, labels_placeholder)
	...
	for epoch in range(Config.epochs): 
		...
		for batch in batches:
			...
			labels_batch = np.array([int(x[2]==3) for x in batch])
...
#			labels_batch = np.array([x[2] for x in batch])
			...
			confusion += confusion_matrix(predictions, labels_batch, labels = range(Config.num_classes))
