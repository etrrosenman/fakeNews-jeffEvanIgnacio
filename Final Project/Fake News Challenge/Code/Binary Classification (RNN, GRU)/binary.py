...
class Config:
	...
	headline_hidden_size = 100
	article_hidden_size = 100
...
def pad_sequences(data, max_length_headline, max_length_articles, reverse = False):
	ret = []
	lengths = []
	...
	for headline, article, labels in data:
		...
		if not reverse:
			...
			copy_headline = headline[::-1]
			copy_article = article[::-1]


		# fix the headline
		headline_len = len(headline) 
		if headline_len >= max_length_headline:
			copy_headline = copy_headline[0:max_length_headline]
		else:
		...
		if article_len >= max_length_articles:
			copy_article = copy_article[0:max_length_articles]
		else:
...
if __name__ == "__main__":
	...
	reversed = False
	print 'Should inputs be reversed?'
	reversedText = raw_input()
	if reversedText.lower() == 'y' or reversedText.lower() == 'yes':
		reversed = True
	...
	for epoch in range(Config.epochs): 
		...
		for batch in batches:
			...
			batch, lengths = pad_sequences(batch, Config.max_length_titles, Config.max_length_articles, reversed)
