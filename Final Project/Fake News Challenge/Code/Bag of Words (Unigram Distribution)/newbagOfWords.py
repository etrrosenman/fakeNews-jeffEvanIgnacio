...
#This function will clean one tuple in the list to remove htl, non-words, and stop words
#Only called for the headlines and articles, takes in a single string (either a headline or
#or an article, and the output is a single string) 

def clean_words(raw_words):
	global clean_count 
	# 1. Remove HTML
	#clean_text = BeautifulSoup([(raw_words).get_text()], lxml) 
	#
	# 2. Remove non-letters       
	letters_only = re.sub("[^a-zA-Z]", " ", raw_words)
	...
	clean_count+=1
	if( (clean_count)%1000 == 0 ):
		print "Number cleaned: %d " % (clean_count)   
...
def load_and_preprocess_data(debug = False):
	...
	if debug:
		allData = [(s['Headline'], articles[s['Body ID']], s['Stance'], s['Body ID']) for s in stances[:200]]
	else:
		allData = [(s['Headline'], articles[s['Body ID']], s['Stance'], s['Body ID']) for s in stances]

	clean_count = 1
	print "Cleaning and parsing all headines and articles...\n"
	newList = [(clean_words(a[0]).split(), clean_words(a[1]).split(), a[2], a[3]) for a in allData]
...
#		allData = [(s['Headline'].split(), articles[s['Body ID']].split(), s['Stance'], s['Body ID']) for s in stances]
	...
	#clean the Data, clean HTML, remove non-words, stop words, and split
	...
	#here, I need to take in allData, which is a tuple of headlines, articles, and labels, and then process and headlines and articles
	#then create the needed vectors
