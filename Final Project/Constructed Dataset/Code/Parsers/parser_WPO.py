...
for file in files:
	...
	with open(sys.argv[1] + file, 'rb') as f:
		...
		for line in lines: 
			...
			if re.match("LOAD-DATE:.*", line) or re.match(".*@washpost.com.*", line):
				active = False 
			...
			if active: 
				...
				if articleMap[2] and not articleMap[3]: 
					...
					# check if it's the section or the length 
					sectionMatch = re.match("SECTION: (.*)", line.strip())
					lengthMatch = re.match("LENGTH: (.*)", line.strip())

					# if not, append to byline 
					if not (sectionMatch or lengthMatch):
					...
					# if length   
						...
						results[len(results) - 1]["Length"] = lengthMatch.group(1)
						articleMap[4] = True 
						continue
...
with open(sys.argv[2], 'wb') as csv_file:
    ...
    for article in results:
    	if (len(article) == 6 and 
    		"washington" in article.get("Publication").lower()):
    		writer.writerow([article.get(i) for i in results[0].keys()])
